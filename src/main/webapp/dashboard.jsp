		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button id="btnExcluirConsulta" type="button"  class="btn btn-danger btn-sm">
						<svg width="1em" height="1em" viewBox="0 0 16 16"
...
<script>
	$(document).ready(function() {
		$("body").append("<div class='d-flex justify-content-center'><div class='spinner-border text-danger' role='status'><span class='sr-only'>Loading...</span></div></div>")
		$.ajax({
			url : "Controller",
			type : 'GET',
			data : {
			...
			success : function(data) {
				...
				for(usuario of data){ 
					...
					var componente = " <tr id='"+ usuario.id+"'> " +
					...
					"<td>" +
						"<button type='button' class='btn btn-secondary btn-sm mr-2'>"+
							...
							"<path fill-rule='evenodd' d='M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5L13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l6.5-6.5zm-9.761 5.175l-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325z'/></svg>"+
						"</button>"+
						"<button onclick='remover("+usuario.id+")' type='button' class='btn btn-danger btn-sm'>"+
	...
	$("#btnBusca").click(function(){
		$.ajax({
			...
			success : function(data) {
				if(data !== null){ 
					$('#buscaModal').modal()
					$('#nomeConsulta').val(data.nome).attr("disabled", true)
					$('#emailConsulta').val(data.email).attr("disabled", true)
					$('#dddConsulta').val(data.telefone[0].ddd).attr("disabled", true)
					$('#numeroConsulta').val(data.telefone[0].numero).attr("disabled", true)
